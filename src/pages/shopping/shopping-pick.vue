  ...
  <div>
    <vp-layout-page>
      ...
      <div class="list-wrapper">
        ...
        :num="item.num"
        :minNum="item.num>0 ? 1 : 0"
        :maxNum="item.isContract === 1 ? (contractNumLimit?contractNumLimit:0) : freeContractNumLimit "
      ...
      <div class="footer-btn" slot="footer">
        ...
        <vp-button class="customer-btn" @click="submit" content="結算"/>
      </div>
    </vp-layout-page>
  </div>
</template>
<script type = 'text/javascript'>
import { vpLiGoodsCart, vpInputNumber, Swipeout, SwipeoutItem, SwipeoutButton } from '@/components'
export default {
  components: { vpLiGoodsCart, Swipeout, SwipeoutItem, SwipeoutButton },
  data() {
    return {
  ...
  created() {
    this.$jb.webview2js((params) => {params.type == 1 && this.$router.go(-1)})
     ...
     this.$event.$on('changeNum',(val)=>{
      console.log('changeNum',val)
      let cdata = this.$session.get('createOrderInfo')
      cdata.productList[0].num = val
        // console.log('cdata',cdata.productList[0].num )
       this.$session.set('createOrderInfo', cdata)
        this.setProList();
    })
  ...
  methods: {
    setProList() {//将商品详情页的数据格式化成此页面需要的格式
      ...
      let [numContract, numNoContract] = [0, 0];
       console.log('contractPriceLimit',contractPriceLimit)
