<template>
  <div>
    ...
    <!-- <popup v-model="showAddress" width="100%" height="100%" position="right" :hide-on-blur="false">
      ...
      <div v-if="showAddress" class="addalert">
      <div class="popup2">
        <div style="position:relative">
          <vpInputAddress v-model="addressVal" v-if="showAddress"/>
        </div>
        <div class="btn-wrapper">
          <vp-button content="確定" @click="slecteAddressBack"/>
        </div>
    ...
    <popup-picker :show="expressShow" :show-cell="false" :data="express" :columns="1" v-model="expressVal" @on-hide="expressHideHandler" class="popupI"></popup-picker>
    <popup-picker :show="addrStoreAreaShow" :show-cell="false" :data="addrStoreArea" :columns="1" v-model="addrStoreAreaVal" @on-change="addrStoreAreaChangeHandler" @on-hide="addrStoreAreaShow = false" ref="addrStoreArea" class="popupII"></popup-picker>
    <popup-picker :show="addrStoreShow" :show-cell="false" :data="addrStore" :columns="1" v-model="addrStoreVal" @on-change="addrStoreChangeHandler" @on-hide="addrStoreShow = false" ref="addrStore" class="popupIII"></popup-picker>
    ...
    methods: {
      ...
      setCreateOrderData() { //整理提交订单需要的数据
        ...
        const deliveryAddress = that.handleType == 2 ? { //收货地址(网站送货才传)
          provinceCode: (addressVal.areaId && addressVal.areaId != undefined ? addressVal.areaId : '') || "", //省编号---區域
          cityCode: (addressVal.districtId && addressVal.districtId != undefined?addressVal.districtId:'' ) || "", //市编号---地區
          districtCode: (addressVal.streetId && addressVal.streetId != undefined? addressVal.streetId:'') || "", //区编号---街道
          province: (addressVal.areaTcResult && addressVal.areaTcResult !=  undefined?addressVal.areaTcResult : '') || "", //省名称
          city: (addressVal.districtTcResult && addressVal.districtTcResult != undefined? addressVal.districtTcResult : '') || "", //市名称
          district: (addressVal.streetTcResult && addressVal.streetTcResult != undefined? addressVal.streetTcResult:'') || "", //区名称
        ...
        const req = {
          ...
          deliveryInfo: { //配送信息 香港移动专用
            ...
            // remark: undefined, //备注
            takeDepartName: (that.handleType == 3 ? that.addrStoreValFormat.storeNameTc : ''), //取货门市名称
            takeDepartCode: (that.handleType == 3 ? that.addrStoreValFormat.id : ''), //取货门市编码
            takeDistrict: (that.handleType == 3 ? that.addrStoreValFormat.districtId : ''), //取货区域
            remark: '', //备注
          },
