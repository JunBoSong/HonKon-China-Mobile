...
export default {
  ...
  methods: {
    async registerOrderInfo() {
      //上台业务单初始化
      if (this.linkParam && this.linkParam.cmhkCustomerId) { //如果是新客或是旗舰机上台会带cmhkCustomerId过来
          ...
          const res = await this.$post("business/init", { //如果是店铺的待机上台/旗舰机上台没有cmhkCustomerId，也需初始化
            businessType: this.linkParam.businessType,
            userId: this.$client.userid
          });
           if (res && res.returnCode == 1000) {
    ...
    async getPhoneNoReserve() { //号码预占(此只新号时要)
      const res = await this.$post("business/phoneNoReserve", {
        "phoneNo": this.phoneSelect,
        "orderNo": this.linkParam && this.linkParam.orderId ? this.linkParam.orderId :this.id
      })
      if (res.returnCode == 1000) {
  ...
  created() {
    ...
    if(this.$client.right == "store") { //根据情况初始化
          if (this.linkParam.businessType == 1 || this.linkParam.businessType == 2 ) { // 带机上台/集客上台
            if (!(this.linkParam && this.linkParam.number)) {
